...
def _SlopesEqual(pt1, pt2, pt3, pt4=None):
    if pt4 is None:
        return (pt1.y - pt2.y) * (pt2.x - pt3.x) == (pt1.x - pt2.x) * (
            pt2.y - pt3.y
        )
    else:
        return (pt1.y - pt2.y) * (pt3.x - pt4.x) == (pt1.x - pt2.x) * (
            pt3.y - pt4.y
        )
...
class ClipperBase(object):
    ...
    def _AddBoundsToLML(self, e):
        ...
        while True:
            if e.dx == horizontal:
                if (e.nextE.Top.y < e.Top.y) and (
                    e.nextE.Bot.x > e.prevE.Bot.x
                ):
                    break
    ...
    def AddPolygon(self, polygon, polyType):
        ...
        while ln > 2:
            if _PointsEqual(pg[j], pg[0]):
                j -= 1
            elif _PointsEqual(pg[0], pg[1]) or _SlopesEqual(
                pg[j], pg[0], pg[1]
            ):
                pg[0] = pg[j]
                j -= 1
...
def _FixHoleLinkage(outRec):
    if outRec.FirstLeft is None or (
        ...
        and outRec.FirstLeft.pts is not None
    ):
        return
...
class Clipper(ClipperBase):
    ...
    def _InsertEdgeIntoAEL(self, edge):
        ...
        if self._ActiveEdges is None:
            ...
            e = self._ActiveEdges
            while e.nextInAEL is not None and not _E2InsertsBeforeE1(
                e.nextInAEL, edge
            ):
                e = e.nextInAEL
    ...
    def _InsertLocalMinimaIntoAEL(self, botY):
        while self._CurrentLocMin is not None and self._CurrentLocMin.y == botY:
            ...
            if self._IsContributing(lb):
                self._AddLocalMinPoly(
                    lb, rb, Point(lb.Curr.x, self._CurrentLocMin.y)
                )

            if (
                rb.outIdx >= 0
                and rb.dx == horizontal
                and self._HorzJoins is not None
            ):
                hj = self._HorzJoins
                while True:
    ...
    def _ProcessHorizontal(self, horzEdge):
        ...
        while e is not None:
            ...
            if (
                ...
                or (
                    (direction == Direction.LeftToRight)
                    and (e.Curr.x < horzRight)
                )
                or (
                    (direction == Direction.RightToLeft)
                    and (e.Curr.x > horzLeft)
                )
            ):
                if e == eMaxPair:
                    if direction == Direction.LeftToRight:
                ...
                direction == Direction.LeftToRight and e.Curr.x >= horzRight
        ...
        if e1Contributing and e2contributing:
            if (
                ...
                or (
                    ...
                    and self._ClipType != ClipType.Xor
                )
            ):
            ...
            if e1.PolyType != e2.PolyType:
                ...
                if self._ClipType == ClipType.Intersection:
                    ...
                    if (
                        e1.PolyType == PolyType.Clip and e1Wc2 > 0 and e2Wc2 > 0
                    ) or (
                        e1.PolyType == PolyType.Subject
                        and e1Wc2 <= 0
                        and e2Wc2 <= 0
                    ):
                        self._AddLocalMinPoly(e1, e2, pt)
...
def _OffsetInternal(
    ...
    def _DoSquare(pt):
        ...
        dx = math.tan(
            math.atan2(
                sinA, Normals[k].x * Normals[j].x + Normals[k].y * Normals[j].y
            )
            / 4
        )
        result.append(
    ...
    def _DoRound(pt):
        ...
        for _ in range(steps):
            result.append(
                Point(round(pt.x + X * delta), round(pt.y + Y * delta))
            )
            X2 = X
            X = X * mcos - msin * Y
            Y = X2 * msin + Y * mcos
    ...
    def _OffsetPoint(jointype):
        if sinA * delta < 0:
            ...
            r = 1.0 + (
                Normals[j].x * Normals[k].x + Normals[j].y * Normals[k].y
            )
            if r >= miterLim:
                _DoMiter(pts[j], r)
            else:
    ...
    if delta > 0:
        c.Execute(
            ClipType.Union, res, PolyFillType.Positive, PolyFillType.Positive
        )
    else:
        ...
        c.Execute(
            ClipType.Union, res, PolyFillType.Negative, PolyFillType.Negative
        )
        if len(res) > 0:
            res.pop(0)
    return res
...
def OffsetPolygons(
    polys, delta, jointype=JoinType.Square, limit=0.0, autoFix=True
):
    if not autoFix:
        return _OffsetInternal(
            polys, True, delta, jointype, EndType.Butt, limit
        )
    pts = polys[:]
    botPoly = None
    botPt = None
    for poly in pts:
        ...
        if (
            botPt is None
            or (bot.y > botPt.y)
            or (bot.y == botPt.y and bot.x < botPt.x)
        ):
            botPt = bot
            botPoly = poly
    if botPt is None:
...
def OffsetPolyLines(
    ...
    if endtype == EndType.Closed:
        ...
        return _OffsetInternal(
            polys2, True, delta, jointype, EndType.Butt, limit
        )
    else:
...
def _ClosestPointOnLine(pt, linePt1, linePt2):
    ...
    q = ((pt.x - linePt1.x) * dx + (pt.Y - linePt1.Y) * dy) / (
        dx * dx + dy * dy
    )
    return FloatPoint(
...
def CleanPolygon(poly, distance=1.415):
    ...
    j = len(result)
    if j > 2 and _SlopesNearColinear(
        result[j - 2], result[j - 1], result[0], distSqrd
    ):
        del result[j - 1 :]
    if len(result) < 3:
        return []
